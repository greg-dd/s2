...
void TestCrossing(const S2Point& a, const S2Point& b,
  ...
  EXPECT_EQ(robust, CrossingSign(a, b, c, d));
  ...
  EXPECT_EQ(edge_or_vertex, EdgeOrVertexCrossing(a, b, c, d));
  crosser.RestartAt(&c);
...
TEST(S2EdgeUtil, Crossings) {
  ...
  // Two edges that cross where one vertex is Origin().
  TestCrossings(S2Point(1, 0, 0), Origin(),
...
TEST(S2EdgeUtil, CollinearEdgesThatDontTouch) {
  ...
  for (int iter = 0; iter < kIters; ++iter) {
    ...
    EXPECT_GT(0, CrossingSign(a, b, c, d));
    EXPECT_GT(0, CrossingSign(a, b, c, d));
    S2EdgeCrosser crosser(&a, &b, &c);
...
TEST(S2EdgeUtil, CoincidentZeroLengthEdgesThatDontTouch) {
  ...
  for (int iter = 0; iter < kIters; ++iter) {
    ...
    // IsUnitLength (which allows for a small amount of error in the norm).
    S2Point a = (1-3e-16) * p;
    S2Point b = (1-1e-16) * p;
    S2Point c = p;
    S2Point d = (1+2e-16) * p;
