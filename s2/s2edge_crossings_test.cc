...
class GetIntersectionStats {
 public:
  GetIntersectionStats() {
    internal::intersection_method_tally_ = tally_;
  }

  ~GetIntersectionStats() {
    internal::intersection_method_tally_ = nullptr;
  }
  ...
  void Print() {
    ...
    for (int i = 0; i < kNumMethods; ++i) {
      ...
      printf("%10s %9d %5.1f%% %9d %5.1f%%  %5.1f%%\n",
             internal::GetIntersectionMethodName(
...
static S2Point GetIntersectionExact(const S2Point& a0, const S2Point& a1,
  ...
  S2Point x = internal::GetIntersectionExact(a0, a1, b0, b1);
...
TEST(S2EdgeUtil, IntersectionError) {
  ...
  for (int iter = 0; iter < 5000; ++iter) {
    ...
    // original point P.
    EXPECT_LE(GetDistance(p, a, b),
              S1Angle::Radians(1.5 * DBL_EPSILON) + kGetDistanceAbsError);
    EXPECT_LE(GetDistance(p, c, d),
              S1Angle::Radians(1.5 * DBL_EPSILON) + kGetDistanceAbsError);
    ...
    EXPECT_LE(GetDistance(expected, a, b),
              S1Angle::Radians(3 * DBL_EPSILON) + kGetDistanceAbsError);
    EXPECT_LE(GetDistance(expected, c, d),
              S1Angle::Radians(3 * DBL_EPSILON) + kGetDistanceAbsError);
    ...
    EXPECT_LE(dist_ab, kIntersectionError + kGetDistanceAbsError);
    EXPECT_LE(dist_cd, kIntersectionError + kGetDistanceAbsError);
    max_edge_dist = max(max_edge_dist, max(dist_ab, dist_cd));
    S1Angle point_dist(expected, actual);
    EXPECT_LE(point_dist, kIntersectionError);
  ...
  for (int iter = 0; iter < 1000; ++iter) {
    ...
    do {
      ...
      ab = (a - b).CrossProd(a + b);
             ...
             CrossingSign(a, b, c, d) <= 0 ||
             CrossingSign(a, b, c, e) <= 0);
    S2Point xcd = GetIntersection(a, b, c, d);
    S2Point xce = GetIntersection(a, b, c, e);
    ...
    ab = ab.Normalize();
    if (S1Angle(xcd, xce) > 2 * kIntersectionError) {
...
TEST(S2EdgeUtil, ExactIntersectionUnderflow) {
  ...
  EXPECT_EQ(S2Point(1, 1e-300, 0), GetIntersection(a0, a1, b0, b1));
}
...
TEST(S2EdgeUtil, GetIntersectionInvariants) {
  ...
  for (int iter = 0; iter < kIters; ++iter) {
    ...
    S2Point result = GetIntersection(a, b, c, d);
