...
// Checks that the error returned by GetUpdateMinDistanceMaxError() for
...
void CheckUpdateMinDistanceMaxError(double actual, double max_error) {
  ...
  S1Angle bound = ca.PlusError(GetUpdateMinDistanceMaxError(ca)).ToAngle();
...
TEST(S2, GetUpdateMinInteriorDistanceMaxError) {
  ...
  for (int iter = 0; iter < 10000; ++iter) {
    ...
    S2Point a1 = InterpolateAtDistance(len, a0, S2Testing::RandomPoint());
    ...
    if (!UpdateMinInteriorDistance(x, a0, a1, &min_dist)) {
      --iter; continue;
    }
    double error = GetUpdateMinDistanceMaxError(min_dist);
...
void CheckDistance(S2Point x, S2Point a, S2Point b,
  ...
  EXPECT_NEAR(distance_radians, GetDistance(x, a, b).radians(), 1e-15);
  S2Point closest = Project(x, a, b);
  if (expected_closest == S2Point(0, 0, 0)) {
    // This special value says that the result should be A or B.
    EXPECT_TRUE(closest == a || closest == b);
  } else {
    EXPECT_TRUE(ApproxEquals(closest, expected_closest));
  }
  S1ChordAngle min_distance = S1ChordAngle::Zero();
  EXPECT_FALSE(UpdateMinDistance(x, a, b, &min_distance));
  min_distance = S1ChordAngle::Infinity();
  EXPECT_TRUE(UpdateMinDistance(x, a, b, &min_distance));
...
void CheckMaxDistance(S2Point x, S2Point a, S2Point b,
  ...
  EXPECT_FALSE(UpdateMaxDistance(x, a, b, &max_distance));
  max_distance = S1ChordAngle::Negative();
  EXPECT_TRUE(UpdateMaxDistance(x, a, b, &max_distance));
...
TEST(S2, Interpolate) {
  ...
  for (double t = 0; t <= 1; t += 0.125) {
    S2Point actual = Interpolate(t, p1, -p1);
...
TEST(S2, InterpolateCanExtrapolate) {
  ...
  S2Point p(Interpolate(0.001, i, j));
...
void CheckEdgePairMinDistance(S2Point a0, S2Point a1, S2Point b0, S2Point b1,
  ...
  const auto& closest = GetEdgePairClosestPoints(a0, a1, b0, b1);
  ...
  if (expected_a == S2Point(0, 0, 0)) {
    ...
    EXPECT_TRUE(ApproxEquals(expected_a, actual_a));
  }
  if (expected_b == S2Point(0, 0, 0)) {
    // This special value says that the result should be b0 or b1.
    EXPECT_TRUE(actual_b == b0 || actual_b == b1);
  } else {
    EXPECT_TRUE(ApproxEquals(expected_b, actual_b));
  }
  S1ChordAngle min_distance = S1ChordAngle::Zero();
  EXPECT_FALSE(UpdateEdgePairMinDistance(a0, a1, b0, b1, &min_distance));
  min_distance = S1ChordAngle::Infinity();
  EXPECT_TRUE(UpdateEdgePairMinDistance(a0, a1, b0, b1, &min_distance));
...
void CheckEdgePairMaxDistance(S2Point a0, S2Point a1, S2Point b0, S2Point b1,
  ...
  S1ChordAngle max_distance = S1ChordAngle::Straight();
  EXPECT_FALSE(UpdateEdgePairMaxDistance(a0, a1, b0, b1, &max_distance));
  max_distance = S1ChordAngle::Negative();
  EXPECT_TRUE(UpdateEdgePairMaxDistance(a0, a1, b0, b1, &max_distance));
  EXPECT_NEAR(distance_radians, max_distance.radians(), 1e-15);
}
...
bool IsEdgeBNearEdgeA(const std::string& a_str, const std::string& b_str,
  ...
  return IsEdgeBNearEdgeA(a->vertex(0), a->vertex(1),
