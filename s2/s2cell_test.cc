...
static void TestSubdivide(const S2Cell& cell) {
  ...
  for (int i = 0; i < 4; ++i, child_id = child_id.next()) {
    ...
    EXPECT_TRUE(ApproxEquals(children[i].GetCenter(), child_id.ToPoint()));
...
static void CheckMinMaxAvg(
    ...
    const Metric<dim>& min_metric,
    const Metric<dim>& max_metric,
    const Metric<dim>& avg_metric) {
...
static S1ChordAngle GetDistanceToPointBruteForce(const S2Cell& cell,
  ...
  for (int i = 0; i < 4; ++i) {
    UpdateMinDistance(target, cell.GetVertex(i),
...
static S1ChordAngle GetMaxDistanceToPointBruteForce(const S2Cell& cell,
  ...
  for (int i = 0; i < 4; ++i) {
    UpdateMaxDistance(target, cell.GetVertex(i),
