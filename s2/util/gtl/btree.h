...
// class is specialized for less<std::string>, greater<std::string>, less<string_view>,
// and greater<string_view>. The
// key_compare_to_adapter is provided so that btree users
// automatically get the more efficient compare-to code when using common
// google std::string types with common comparison functors.
...
struct map_params : common_params<Key, Compare, Alloc, TargetNodeSize,
  ...
  using slot_policy = absl::container_internal::map_slot_policy<Key, mapped_type>;
...
struct set_params
  ...
  struct slot_policy {
    ...
    static void construct(Alloc *alloc, slot_type *slot, slot_type *other) {
      slot_type::construct(alloc, slot, other);
    }
...
class btree_node {
  ...
  using slot_policy = typename Params::slot_policy;
 ...
 private:
  ...
  void value_init(const size_type i, allocator_type *alloc, Args &&... args) {
    ...
    slot_policy::construct(alloc, slot(i), std::forward<Args>(args)...);
  }
  void value_destroy(const size_type i, allocator_type *alloc) {
    slot_policy::destroy(alloc, slot(i));
  ...
  void uninitialized_move_n(const size_type n, const size_type i,
    ...
    for (slot_type *src = slot(i), *end = src + n, *dest = x->slot(j);
         src != end; ++src, ++dest) {
      slot_policy::construct(alloc, dest, src);
    }
  }
...
class btree {
 ...
 private:
  ...
  // speeding up std::string, cord and string_view keys.
...
inline void btree_node<P>::emplace_value(const size_type i,
  ...
  if (i < count()) {
    ...
    for (size_type j = count() - 1; j > i; --j)
      slot_policy::move(alloc, slot(j - 1), slot(j));
    value_destroy(i, alloc);
  }
...
inline void btree_node<P>::remove_value(const int i, allocator_type *alloc) {
  ...
  slot_policy::move(alloc, slot(i + 1), slot(count()), slot(i));
...
void btree_node<P>::rebalance_right_to_left(const int to_move,
  ...
  slot_policy::move(alloc, right->slot(to_move - 1), parent()->slot(position()));

  // 4) Shift the values in the right node to their correct position.
  slot_policy::move(alloc, right->slot(to_move), right->slot(right->count()),
                  right->slot(0));
...
void btree_node<P>::rebalance_left_to_right(const int to_move,
  ...
  if (right->count() >= to_move) {
    ...
    slot_policy::move(alloc, parent()->slot(position()),
                    right->slot(to_move - 1));

    // 3) Move the (to_move - 1) values from the left node to the right node.
    slot_policy::move(alloc, slot(count() - (to_move - 1)), slot(count()),
                    right->slot(0));
  } else {
    ...
    uninitialized_move_n(uninitialized_remaining,
                         ...
                         right, alloc);
    slot_policy::move(alloc, slot(count() - (to_move - 1)),
                    slot(count() - uninitialized_remaining), right->slot(0));
  }

  // 4) Move the new delimiting value to the parent from the left node.
  slot_policy::move(alloc, slot(count() - to_move), parent()->slot(position()));
...
auto btree<P>::erase(iterator iter) -> iterator {
  ...
  if (!iter.node->leaf()) {
    ...
    slot_policy::move(mutable_allocator(), iter.node->slot(iter.position),
