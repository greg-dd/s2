...
class ExactFloat {
 public:
  ...
  // Construct an ExactFloat from a std::string (such as "1.2e50").  Requires that
  ...
  // Return a human-readable std::string such that if two values with the same
  // precision are different, then their std::string representations are different.
  ...
  // Return a std::string formatted according to printf("%Ng") where N is the given
  // maximum number of significant digits.
  std::string ToStringWithMaxDigits(int max_digits) const;

  // Return a human-readable std::string such that if two ExactFloats have different
  // values, then their std::string representations are always different.  This
  // method is useful for debugging.  The std::string has the form "value<prec>",
  ...
  // they are formatted as decimal std::strings in exponential format.
