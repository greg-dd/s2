...
TEST(S2, AngleMethods) {
  ...
  EXPECT_DOUBLE_EQ(Angle(p000, pz, p045), M_PI_4);
  EXPECT_DOUBLE_EQ(TurnAngle(p000, pz, p045), -3 * M_PI_4);

  EXPECT_DOUBLE_EQ(Angle(p045, pz, p180), 3 * M_PI_4);
  EXPECT_DOUBLE_EQ(TurnAngle(p045, pz, p180), -M_PI_4);

  EXPECT_DOUBLE_EQ(Angle(p000, pz, p180), M_PI);
  EXPECT_DOUBLE_EQ(TurnAngle(p000, pz, p180), 0);

  EXPECT_DOUBLE_EQ(Angle(pz, p000, p045), M_PI_2);
  EXPECT_DOUBLE_EQ(TurnAngle(pz, p000, p045), M_PI_2);

  EXPECT_DOUBLE_EQ(Angle(pz, p000, pz), 0);
  EXPECT_DOUBLE_EQ(fabs(TurnAngle(pz, p000, pz)), M_PI);
}
...
TEST(S2, AreaMethods) {
  ...
  EXPECT_DOUBLE_EQ(Area(p000, p090, pz), M_PI_2);
  EXPECT_DOUBLE_EQ(Area(p045, pz, p180), 3 * M_PI_4);
  ...
  EXPECT_NEAR(Area(pepsx, pepsy, pz), expected1, 1e-14 * expected1);
  ...
  EXPECT_NEAR(Area(pr, pq, pr), 0, 1e-15);
  EXPECT_EQ(Area(p000, p045, p090), 0);
  ...
  for (int i = 0; i < 10000; ++i) {
    ...
    EXPECT_LE(Area(p0, p1, p2), 0.7e-30);
    max_girard = std::max(max_girard, GirardArea(p0, p1, p2));
  }
  ...
  EXPECT_NEAR(Area(p000, p045eps, p090), expected2, 1e-9 * expected2);
  ...
  // This tests a case where the triangle has zero area, but Area()
  // computes (dmin > 0) due to rounding errors.
  EXPECT_EQ(0.0, Area(S2LatLng::FromDegrees(-45, -170).ToPoint(),
