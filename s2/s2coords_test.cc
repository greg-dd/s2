...
TEST(S2, ST_UV_Conversions) {
  ...
  for (double s = 0; s <= 1; s += 0.5) {
    volatile double u = STtoUV(s);
    EXPECT_EQ(u, 2 * s - 1);
  }
  for (double u = -1; u <= 1; ++u) {
    volatile double s = UVtoST(u);
    EXPECT_EQ(s, 0.5 * (u + 1));
  }
  // Check that UVtoST and STtoUV are inverses.
  for (double x = 0; x <= 1; x += 0.0001) {
    EXPECT_NEAR(UVtoST(STtoUV(x)), x, 1e-15);
    EXPECT_NEAR(STtoUV(UVtoST(2*x-1)), 2*x-1, 1e-15);
  }
}
...
TEST(S2, FaceUVtoXYZ) {
  ...
  for (int face = 0; face < 6; ++face) {
    EXPECT_EQ(GetUAxis(face).CrossProd(GetVAxis(face))
              .DotProd(FaceUVtoXYZ(face, 0, 0)), 1);
  }
  ...
  for (int face = 0; face < 6; ++face) {
    ...
    EXPECT_EQ(FaceUVtoXYZ(face, sign, -sign),
              FaceUVtoXYZ((face + 1) % 6, -1, -1));
  }
}

TEST(S2, FaceXYZtoUVW) {
  for (int face = 0; face < 6; ++face) {
    EXPECT_EQ(S2Point( 0,  0,  0), FaceXYZtoUVW(face, S2Point(0, 0, 0)));
    EXPECT_EQ(S2Point( 1,  0,  0), FaceXYZtoUVW(face,  GetUAxis(face)));
    EXPECT_EQ(S2Point(-1,  0,  0), FaceXYZtoUVW(face, -GetUAxis(face)));
    EXPECT_EQ(S2Point( 0,  1,  0), FaceXYZtoUVW(face,  GetVAxis(face)));
    EXPECT_EQ(S2Point( 0, -1,  0), FaceXYZtoUVW(face, -GetVAxis(face)));
    EXPECT_EQ(S2Point( 0,  0,  1), FaceXYZtoUVW(face,  GetNorm(face)));
    EXPECT_EQ(S2Point( 0,  0, -1), FaceXYZtoUVW(face, -GetNorm(face)));
  }
}
...
TEST(S2, XYZToFaceSiTi) {
  ...
  for (int level = 0; level <= S2CellId::kMaxLevel; ++level) {
    for (int i = 0; i < 1000; ++i) {
      ...
      actual_level = XYZtoFaceSiTi(p_moved, &face_moved, &si_moved,
                                       &ti_moved);
      ...
      } while (si_random > kMaxSiTi || ti_random > kMaxSiTi);
...
TEST(S2, UVNorms) {
  ...
  for (int face = 0; face < 6; ++face) {
    for (double x = -1; x <= 1; x += 1/1024.) {
      EXPECT_DOUBLE_EQ(FaceUVtoXYZ(face, x, -1)
                       .CrossProd(FaceUVtoXYZ(face, x, 1))
                       .Angle(GetUNorm(face, x)), 0);
      EXPECT_DOUBLE_EQ(FaceUVtoXYZ(face, -1, x)
                       .CrossProd(FaceUVtoXYZ(face, 1, x))
                       .Angle(GetVNorm(face, x)), 0);
    }
  }
}
...
TEST(S2, UVWAxis) {
  for (int face = 0; face < 6; ++face) {
    ...
    EXPECT_EQ(FaceUVtoXYZ(face, 1, 0) - FaceUVtoXYZ(face, 0, 0),
              GetUAxis(face));
    EXPECT_EQ(FaceUVtoXYZ(face, 0, 1) - FaceUVtoXYZ(face, 0, 0),
              GetVAxis(face));
    EXPECT_EQ(FaceUVtoXYZ(face, 0, 0), GetNorm(face));

    // Check that every face coordinate frame is right-handed.
    EXPECT_EQ(1, GetUAxis(face).CrossProd(GetVAxis(face))
              .DotProd(GetNorm(face)));

    // Check that GetUVWAxis is consistent with GetUAxis, GetVAxis, GetNorm.
    EXPECT_EQ(GetUAxis(face), GetUVWAxis(face, 0));
    EXPECT_EQ(GetVAxis(face), GetUVWAxis(face, 1));
    EXPECT_EQ(GetNorm(face), GetUVWAxis(face, 2));
  }
}
...
TEST(S2, UVWFace) {
  ...
  for (int face = 0; face < 6; ++face) {
    for (int axis = 0; axis < 3; ++axis) {
      EXPECT_EQ(GetFace(-GetUVWAxis(face, axis)),
                GetUVWFace(face, axis, 0));
      EXPECT_EQ(GetFace(GetUVWAxis(face, axis)),
                GetUVWFace(face, axis, 1));
    }
  }
}
